...
public enum CodeTemplateEnum {
  ...
  VO_FILE_PATH("%s%sdto%sresp%s"),
  SERVICE_FILE_PATH("%s%sservice%s"),
  SERVICE_IMPL_FILE_PATH("%s%sservice%simpl%s"),
  QUERY_FILE_PATH("%s%sdto%sreq%squery%s"),
  ADD_FILE_PATH("%s%sdto%sreq%sadd%s"),
  UPDATE_FILE_PATH("%s%sdto%sreq%supdate%s"),
  /** 生成模板路径 */
