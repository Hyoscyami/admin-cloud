 ...
 */
@Data
@ApiModel(value = "客户端基本信息", description = "客户端基本信息展示层")
...
public class ClientVO extends BaseVO{
     ...
     */
    @ApiModelProperty(value = "客户端认证方式：client_secret_basic(http basic认证),client_secret_post(post提交认证),client_secret_jwt(jwt认证),private_key_jwt(私钥jwt认证),none(不认证),多个的情况以","分割")
    private String authenticationMethod;

    /**
     * 授权方式：authorization_code(授权码),refresh_token,client_credentials(客户端授权),password(密码授权),jwt_bearer
     */
    @ApiModelProperty(value = "授权方式：authorization_code(授权码),refresh_token,client_credentials(客户端授权),password(密码授权),jwt_bearer")
